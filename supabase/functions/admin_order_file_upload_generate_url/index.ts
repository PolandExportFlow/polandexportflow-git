...
serve(async (req) => {
  ...
  try {
    ...
    // INSERT do bazy (Jako Admin) - tabela: order_files
    const { error: dbInsertError } = await supabase
      .from('order_files')
      .insert({
        order_id: order_id,
        user_id: user.id, // Zapisujemy ID Admina jako autora
    ...
    return new Response(
      JSON.stringify({ presignedUrl, orderId: order_id, storagePath: storage_path }),
