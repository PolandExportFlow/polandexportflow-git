...
serve(async (req) => {
  ...
  try {
    ...
    // 🛑 ZMIANA: Tutaj oczekujemy item_id
    ...
    // 1. Sprawdzenie własności przez tabelę order_items
    ...
    if (itemError || !itemRow) {
      return new Response(JSON.stringify({ error: 'Item not found' }), { headers: corsHeaders, status: 404 })
    }

    // @ts-ignore
    const ownerId = itemRow.orders?.user_id

    if (ownerId !== user.id) {
      return new Response(JSON.stringify({ error: 'Not allowed for this item' }), { headers: corsHeaders, status: 403 })
    }

    const fileName = storage_path.split('/').pop() || 'file'

    // 2. Insert do order_items_files (Atomic Upload)
    const { error: dbInsertError } = await supabase
      .from('order_items_files') // <--- Tabela dla plików przedmiotów
      .insert({
        item_id: item_id,
        user_id: user.id,
