...
serve(async (req) => {
  ...
  try {
    ...
    // 1. Pobierz ścieżkę z bazy (Tabela: order_files)
    ...
    if (!row) {
      // Jeśli nie ma rekordu, to znaczy że już usunięty lub nie istnieje
    ...
    console.log(`Deleting from R2 (General): ${row.storage_path}`)
