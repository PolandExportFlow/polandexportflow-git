...
export async function POST(req: Request) {
  try {
    ...
    // akceptuj 'lookup' i 'p_lookup' (kompatybilnie ze starym klientem)
    const lookup: string = String(body?.lookup ?? body?.p_lookup ?? '')
      .trim()
      .replace(/^#/, '')
    ...
    // musi być zalogowany user – auth.uid() w SQL i tak to sprawdza
    const { data: authData } = await supabase.auth.getUser()
    if (!authData?.user) {
      return NextResponse.json({ error: 'Unauthorized' }, { status: 401 })
    }

    const { data, error } = await supabase.rpc('user_order_update_address', {
      p_lookup: lookup,
      p_patch: patch,
    })
