...
// Typ pomocniczy, żeby nie importować całego hooka, jeśli potrzebujemy tylko format
type PaymentHookMinimal = {
...
type Props = {
    ...
    totalAmount: number
    payment: PaymentHookMinimal // ✅ ZMIANA NAZWY PROPSA (było checkout)
...
export default function PaymentButtons({
    ...
    serviceAmount,
    totalAmount,
    payment, // ✅ Odbieramy prop 'payment'
    onOpenModal,
}: Props) {
            ...
            <div className='flex flex-col gap-2 mt-4'>
                {showProductFields && (
                    <button type='button' onClick={() => onOpenModal(productsAmount)} className={greenButtonClass}>
                        <div className='flex items-center gap-4'>
                            ...
                            <ButtonLabel label='Pay for Items' amount={payment.format(productsAmount)} />
                        </div>
                    </button>
                )}
                <button type='button' onClick={() => onOpenModal(serviceAmount)} className={greenButtonClass}>
                    <div className='flex items-center gap-4'>
                        ...
                        <ButtonLabel label='Pay for Service' amount={payment.format(serviceAmount)} />
                    </div>
                </button>
                {showPayAllCondition && (
                    <button type='button' onClick={() => onOpenModal(totalAmount)} className={greenButtonClass}>
                        <div className='flex items-center gap-4'>
                            ...
                            <ButtonLabel label='Pay All' amount={payment.format(totalAmount)} />
                        </div>
                    </button>
                )}
            </div>
