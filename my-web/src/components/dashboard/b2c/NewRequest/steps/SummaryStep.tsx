...
export default function SummaryStep({
    ...
    // Obliczanie sum przy użyciu helperów
    const totals = useMemo(() => {
        const itemsCount = items.length
        const quantity = items.reduce((s, i) => s + toQty(i.item_quantity), 0)
        const totalValue = items.reduce((s, i) => s + toNumOrZero(i.item_value), 0)
                ...
                <div className='space-y-3 mt-3'>
                    {items.length === 0 ? (
                        ...
                        items.map((it, idx) => {
                            ...
                            const price = toNumOrZero(it.item_value)
                            const qty = toQty(it.item_quantity)
...
/* ===== Sub-components (bez zmian logiki, tylko style/typ) ===== */
